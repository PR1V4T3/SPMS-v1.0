...
bool fileStatus(bool isShowInformation = false){                                // DONE
    ...
    bool status = checkIsExist("TEMPLATE.txt") && checkIsExist("TEMPORARY.txt") && checkIsExist("PARSER_CONFIG.txt") && configFileStatus();
        ...
        if(checkIsExist("PARSER_CONFIG.txt") == true){
...
const char *dateAndTime(string mode = "all"){                                   // TODO: Downloading from system, hard to do
...
int main(){
    ...
    if(configFileStatus() == true) runApplication();
